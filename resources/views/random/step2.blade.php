			   ...
			   @if ($errors->any())
                    <ul class="mt-1.5 list-disc list-inside ">
                        @foreach ($errors->all() as $error)
                            <li class="ms-2 text-sm font-medium  mb-3 flex border border-green-200 text-red-500">لطفا حداقل 5 تصویر را انتخاب کنید</li>
                        @endforeach
                    </ul>
                @endif
                <div class="bg-green-600 shadow-xl rounded-xl">
                    <div class="bg-green-400 px-4 py-3 rounded-t-lg text-2xl font-bold text-center">تبلیغات مرحله (2/4)</div>

                    <div class="pr-8 py-6 ml-5">

                      
                  
                        <form action="{{ route('random.step.store',['step'=>3]) }}" method="POST">
                            @csrf

                                    <input type="hidden" name="advertise" value="{{ $imagePath }}">
                                ...
                                <ul class="grid p-3 grid-cols-3 mr-0 md:grid-cols-5 gap-6">
                                    @foreach($randomImages as $id=>$image)

                                        <li>
                                            <input type="checkbox" name="selected_images[]" id="react-option{{$id}}"
