			    ...
			    @if ($errors->any())
                    <ul class="mt-1.5 list-disc list-inside ">
                        @foreach ($errors->all() as $error)
                            <li class="ms-2 text-sm font-medium text-red-500">لطفا حداقل 5 تصویر را انتخاب کنید</li>
                        @endforeach
                    </ul>
                @endif
                <div class="bg-green-600 shadow-xl rounded-xl">
                    <div class="bg-green-400 px-4 py-3 rounded-t-lg text-2xl font-bold text-center">تبلیغات مرحله (3/4)</div>

                    <div class="pr-8 py-6 ml-5">

                      
                  
                        <form action="{{ route('random.step.store',['step'=>4]) }}" method="POST">
                            @csrf
